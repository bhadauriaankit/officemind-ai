...
import { useCurrentUser } from "@/features/system/useCurrentUser";
...
export function ChatPage() {
  ...
  const { data: conversations, isLoading: isLoadingList } = useConversationList();
  ...
  }, [activeConversation?.messages.length, isSending, pendingMessage]);
    ...
    if (!activeId) {
      startConversation.mutate(
        { message, agentId: selectedAgentId || null },
        {
          ...
          onError: () => {
            // Keep input recoverable if failed
            setInput(message);
            setPendingMessage(null);
          },
        }
  ...
  const userName = profile?.displayName || (auth.user?.profile?.name as string | undefined) || "Employee";
    ...
    <div className="flex h-screen bg-slate-50 font-sans overflow-hidden">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl bg-white shadow-2xl border border-slate-200 p-6">
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeletingId(null)} className="flex-1 rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors">
                Cancel
              </button>
      ...
      <aside className="w-64 bg-slate-900 text-slate-300 flex flex-col shrink-0 z-20">
        ...
        <div className="p-4">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-3 text-sm font-bold text-white shadow-md hover:bg-blue-500 transition-all active:scale-95"
          >
        ...
        <div className="px-4 pb-2">
          <Link to="/search" className="flex items-center gap-3 rounded-xl px-3 py-2 text-sm text-slate-400 hover:bg-slate-800 hover:text-white transition-all mb-1">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
            Search Documents
          </Link>
          <Link to="/admin" className="flex items-center gap-3 rounded-xl px-3 py-2 text-sm text-slate-400 hover:bg-slate-800 hover:text-white transition-all mb-1">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="flex-1 overflow-y-auto px-4 custom-scrollbar">
          ...
          {conversations?.map((c) => (
            <div
              ...
              className={`group relative flex items-center rounded-xl mb-1 transition-all ${
                c.id === activeId ? "bg-slate-800 text-white font-semibold" : "text-slate-400 hover:bg-slate-800/50 hover:text-slate-200"
              }`}
            >
              <button onClick={() => { setActiveId(c.id); setPendingMessage(null); }} className="flex-1 flex items-center gap-2.5 px-3 py-2.5 text-left text-sm truncate min-w-0">
                <svg className={`w-3.5 h-3.5 shrink-0 ${c.id === activeId ? "text-blue-400" : "text-slate-600"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {conversations?.length === 0 && !isLoadingList && (
            <p className="px-3 py-4 text-xs text-slate-600 italic">No previous chats. Start one below!</p>
          )}
        </div>
        ...
        <div className="p-4 border-t border-white/10 bg-slate-900">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-blue-600 to-indigo-700 flex items-center justify-center text-xs font-bold text-white shrink-0 shadow-sm">
              {userInitials}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-bold text-slate-200 truncate">{userName}</p>
              <p className="text-[10px] text-slate-400 font-medium truncate">
                {userRoles.includes("ADMIN") ? "Administrator" : "Employee"}
              </p>
            </div>
            <button
              onClick={handleLogout}
              title="Sign out of Keycloak session"
      ...
      <div className="flex flex-1 flex-col relative bg-slate-50/50 overflow-hidden">
        <header className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 bg-white/90 backdrop-blur-md px-8 py-3.5 shadow-sm">
          ...
          <div className="flex items-center gap-3">
            <Link to="/search" className="text-sm font-semibold text-slate-600 hover:text-slate-900 bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-xl transition-colors">
              Search
            </Link>
            <NotificationBell />
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-8 custom-scrollbar">
          {!activeConversation && !pendingMessage && (
            <div className="flex flex-col items-center justify-center p-8 text-center min-h-[60vh]">
              <div className="w-20 h-20 rounded-3xl bg-blue-50 border-2 border-blue-100 flex items-center justify-center mb-6 shadow-inner">
                <svg className="w-10 h-10 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
                </svg>
              </div>
              <h3 className="text-2xl font-extrabold text-slate-900 mb-2">How can I help you today?</h3>
              <p className="text-slate-500 max-w-sm text-sm mb-8">
                Ask questions regarding company policies, deployment architecture, or consult specialized assistants.
              </p>
              {agents && agents.length > 0 && (
                <div className="w-full max-w-xs bg-white rounded-2xl border border-slate-200 shadow-sm p-4 text-left">
          ...
          <div className="mx-auto max-w-3xl space-y-6 pb-4">
            ...
            {/* Optimistic user bubble while awaiting first conversation reply */}
            ...
            {isSending && (
              <div className="flex justify-start">
                <div className="flex gap-3 max-w-[85%]">
                  ...
                  <div className="rounded-2xl rounded-tl-none bg-white border border-slate-200 px-5 py-4 shadow-sm flex items-center gap-2">
                    <span className="text-xs font-semibold text-slate-500">OfficeMind is thinking</span>
        ...
        <div className="bg-white border-t border-slate-200 px-6 py-4">
          {activeError && (
            <div className="mb-3 mx-auto max-w-3xl flex items-center justify-between rounded-xl border border-red-200 bg-red-50 px-4 py-2.5 text-sm text-red-700 shadow-sm">
              <div className="flex items-center gap-2">
                <svg className="w-4 h-4 text-red-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span className="font-semibold">{extractErrorMessage(activeError)}</span>
              </div>
              <button
                onClick={() => { startConversation.reset(); sendMessage.reset(); }}
          ...
          <form onSubmit={handleSubmit} className="mx-auto max-w-3xl flex gap-3">
            <input
              ...
              placeholder={isSending ? "Waiting for response…" : "Message OfficeMind AI…"}
              disabled={isSending}
              className="flex-1 rounded-xl border border-slate-200 bg-slate-50 px-5 py-3 text-sm text-slate-800 placeholder-slate-400 focus:border-blue-500 focus:bg-white focus:outline-none focus:ring-4 focus:ring-blue-500/10 disabled:opacity-60 transition-all font-medium"
            />
            <button
              type="submit"
          ...
          <p className="text-center mt-2 text-[11px] text-slate-400">
            Private enterprise AI assistant. Responses are generated locally via self-hosted models.
          </p>
        </div>
      </div>
    </div>
  ...
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div className={`flex gap-3 max-w-[85%] ${isUser ? "flex-row-reverse" : "flex-row"}`}>
        <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 text-xs font-bold shadow-sm ${isUser ? "bg-slate-200 text-slate-700" : "bg-blue-600 text-white"}`}>
          {isUser ? "ME" : (
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
